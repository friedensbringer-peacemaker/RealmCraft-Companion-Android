...
public final class SnapshotDiff {
    ...
    public static SnapshotDiff between(SnapshotAnalysis before,SnapshotAnalysis after){
        ...
        for(MapPoint p:after.points)if(p.kind.equals("chest")){MapPoint old=chests.remove(key(p.dimension,p.x,p.y,p.z));if(old==null||!old.readable||!p.readable)d.unknownChests++;else if(!items(old).equals(items(p)))d.changedChests++;}d.unknownChests+=chests.size();return d;
    }
