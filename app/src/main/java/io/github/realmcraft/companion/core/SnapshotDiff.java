...
public final class SnapshotDiff {
    ...
    public final Map<String,boolean[]> overlay=new HashMap<>();
    ...
    public static SnapshotDiff between(SnapshotAnalysis before,SnapshotAnalysis after){
        ...
        for(ChunkSurface c:after.chunks){ChunkSurface old=chunks.remove(key(c.dimension,c.x,0,c.z));boolean[] mask=new boolean[256];if(old==null){d.addedChunks++;Arrays.fill(mask,true);}else for(int i=0;i<256;i++)if(c.ids[i]!=old.ids[i]||c.heights[i]!=old.heights[i]){d.changedColumns++;mask[i]=true;}d.overlay.put(c.dimension+":"+c.x+":"+c.z,mask);}d.missingChunks=chunks.size();
