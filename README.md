...
**Status: 0.1.1 experimental.** This is a metadata and file-import test app, not the complete Mac Companion. It is not affiliated with Tellurion Mobile or Meta.
...
- **Quest:** enable developer mode and authorize USB debugging, then install with SideQuest or `adb install -r RealmCraft-Companion-Android-0.1.1.apk`. Open **! ★ RealmCraft Companion Lab** from the headset's installed/unknown-source apps. The exact launcher label depends on Horizon OS.
